defmodule AshUUID.EncodedV7 do
  @constraints [
  ...
  def generator(_constraints) do
    string_uuid = AshUUID.generate(7)
    {:ok, b62_string_uuid} = AshUUID.UUID.encode62(string_uuid)
    b62_string_uuid
  end
    ...
    case AshUUID.format?(term) do
      :prefixed_b62_string_uuid ->
        with true <- AshUUID.valid?(term) do
          [_prefix, b62_string_uuid] = String.split(term, "_")
          {:ok, b62_string_uuid}
        else
          _ -> :error
        end
      ...
      :integer_uuid ->
        with true <- AshUUID.valid?(term) do
          ...
          AshUUID.UUID.encode62(string_uuid)
        else
          _ -> :error
        end
