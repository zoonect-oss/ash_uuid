defmodule AshUUID.RawV4Test do
  ...
  describe "AshUUID.RawV4" do
    ...
    test "initial testing" do
      ...
      assert {:ok, ^uuid_raw} =
               Ash.Type.apply_constraints(AshUUID.RawV4, uuid_raw, prefix: prefix)
      ...
      assert {:ok, ^uuid_string} =
               Ash.Type.cast_input(AshUUID.RawV4, uuid_prefixed_base62_string, prefix: prefix)

      assert {:ok, ^uuid_string} =
               Ash.Type.cast_input(AshUUID.RawV4, uuid_base62_string, prefix: prefix)
      ...
      assert {:ok, ^uuid_string} =
               Ash.Type.cast_stored(AshUUID.RawV4, uuid_string, prefix: prefix)

      assert {:ok, ^uuid_string} =
               Ash.Type.cast_stored(AshUUID.RawV4, uuid_base62_string, prefix: prefix)

      assert {:ok, ^uuid_string} =
               Ash.Type.cast_stored(AshUUID.RawV4, uuid_prefixed_base62_string, prefix: prefix)

      # from elixir type instance to postgresl
      assert {:ok, ^uuid_raw} =
               Ash.Type.dump_to_native(AshUUID.RawV4, uuid_prefixed_base62_string, prefix: prefix)

      assert {:ok, ^uuid_raw} =
               Ash.Type.dump_to_native(AshUUID.RawV4, uuid_base62_string, prefix: prefix)

      assert {:ok, ^uuid_raw} =
               Ash.Type.dump_to_native(AshUUID.RawV4, uuid_string, prefix: prefix)
