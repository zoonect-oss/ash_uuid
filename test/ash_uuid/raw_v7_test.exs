defmodule AshUUID.RawV7Test do
  ...
  describe "AshUUID.RawV7" do
    ...
    test "initial testing" do
      ...
      assert {:ok, ^uuid_raw} = Ash.Type.apply_constraints(AshUUID.RawV7, uuid_raw, [prefix: prefix])

      assert true == Ash.Type.ash_type?(AshUUID.RawV7)
      assert false == Ash.Type.builtin?(RawV7)

      # from elixir to elixir type instance
      assert {:ok, ^uuid_string} = Ash.Type.cast_input(AshUUID.RawV7, uuid_prefixed_base62_string, [prefix: prefix])
      assert {:ok, ^uuid_string} = Ash.Type.cast_input(AshUUID.RawV7, uuid_base62_string, [prefix: prefix])
      ...
      assert {:ok, ^uuid_string} = Ash.Type.cast_stored(AshUUID.RawV7, uuid_string, [prefix: prefix])
      assert {:ok, ^uuid_string} = Ash.Type.cast_stored(AshUUID.RawV7, uuid_base62_string, [prefix: prefix])
      assert {:ok, ^uuid_string} = Ash.Type.cast_stored(AshUUID.RawV7, uuid_prefixed_base62_string, [prefix: prefix])

      # from elixir type instance to postgresl
      assert {:ok, ^uuid_raw} = Ash.Type.dump_to_native(AshUUID.RawV7, uuid_prefixed_base62_string, [prefix: prefix])
      assert {:ok, ^uuid_raw} = Ash.Type.dump_to_native(AshUUID.RawV7, uuid_base62_string, [prefix: prefix])
      assert {:ok, ^uuid_raw} = Ash.Type.dump_to_native(AshUUID.RawV7, uuid_string, [prefix: prefix])
